...
RSpec.describe WebtoonSource::KaynScan do
  let(:base_url) { "https://kaynscan.org/" }
  ...
  describe "#panels" do
    let(:series_slug) { "swordmasters-return" }
    let(:chapter_number) { "4" }
    let(:fixture_path) { File.expand_path("../fixtures/kayn_scan/chapter.html", __dir__) }
    ...
    it "returns the CDN base_url and the full panel_list" do
      ...
      expect(panels.base_url).to eq("https://cdn.kaynscan.org")
      expect(panels.panel_list.size).to eq(3)
    end

    it "orders each panel by its reader index and captures its file extension" do
      first_panel_path = "/public/upload/series/swordmasters-return/chapter_4/1_1700000000.webp"
    ...
    context "with a specific chapter_url" do
      let(:chapter_url) { "https://kaynscan.org/series/swordmasters-return/chapter-4/" }
  ...
  describe "#chapters" do
    let(:series_slug) { "swordmasters-return" }
    let(:fixture_path) { File.expand_path("../fixtures/kayn_scan/series.html", __dir__) }
    let(:html_content) { File.read(fixture_path) }
    ...
    it "returns chapters sorted by number, descending" do
      ...
      expect(chapters.map(&:number)).to eq(%w[5 4])
    end
    ...
    it "builds the chapter path from the series slug and chapter slug" do
      chapter = source.chapters.first
      expect(chapter.path).to eq("/series/#{series_slug}/chapter-5")
    end

    context "with a specific series_url" do
      let(:series_url) { "https://kaynscan.org/series/swordmasters-return/" }
  ...
  describe "#download" do
    ...
    let(:cdn_base_url) { "https://cdn.kaynscan.org" }
    ...
    context "with a specific chapter_url" do
      ...
      it "derives series_slug, chapter_number, and directory_name from the URL" do
        ...
        chapter_directory = File.join(storage_path, "swordmasters_return", "4")
