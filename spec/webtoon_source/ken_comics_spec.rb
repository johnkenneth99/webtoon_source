...
RSpec.describe WebtoonSource::KenComics do
  let(:base_url) { "https://kencomics.com/" }
  ...
  describe "#panels" do
    let(:series_slug) { "villain-to-kill" }
    let(:chapter_number) { "7" }
    let(:fixture_path) { File.expand_path("../fixtures/ken_comics/chapter.html", __dir__) }
    ...
    it "returns the CDN base_url and the full panel_list" do
      ...
      expect(panels.base_url).to eq("https://cdn.kencomics.com")
      expect(panels.panel_list.size).to eq(3)
    end

    it "orders each panel by its reader index and captures its file extension" do
      first_panel_path = "/public/upload/series/villain-to-kill/chapter_7/1_1710000000.webp"
    ...
    context "with a specific chapter_url" do
      let(:chapter_url) { "https://kencomics.com/series/villain-to-kill/chapter-7/" }
  ...
  describe "#chapters" do
    let(:series_slug) { "villain-to-kill" }
    let(:fixture_path) { File.expand_path("../fixtures/ken_comics/series.html", __dir__) }
    let(:html_content) { File.read(fixture_path) }
    ...
    it "returns chapters sorted by number, descending" do
      ...
      expect(chapters.map(&:number)).to eq(%w[8 7])
    end
    ...
    it "builds the chapter path from the series slug and chapter slug" do
      chapter = source.chapters.first
      expect(chapter.path).to eq("/series/#{series_slug}/chapter-8")
    end

    context "with a specific series_url" do
      let(:series_url) { "https://kencomics.com/series/villain-to-kill/" }
  ...
  describe "#download" do
    ...
    let(:cdn_base_url) { "https://cdn.kencomics.com" }
    ...
    context "with a specific chapter_url" do
      ...
      it "derives series_slug, chapter_number, and directory_name from the URL" do
        ...
        chapter_directory = File.join(storage_path, "villain_to_kill", "7")
